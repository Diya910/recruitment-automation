...
class MasterAgentAction(str, Enum):
    """Possible actions for the master agent."""
...
async def initialize_interview(state: MasterAgentState, scenario_manager, conversation_engine) -> MasterAgentState:
...
async def process_response(state: MasterAgentState, conversation_engine) -> MasterAgentState:
...
async def run_evaluation_tools(state: MasterAgentState, tools) -> MasterAgentState:
    ...
    try:
        ...
        if not tools_to_run or "summarize_interview_history" in tools_to_run:
            ...
            try:
                analysis_state = await tools["summarize_interview_history"](analysis_state)
        ...
        if (not tools_to_run or "grammar_check" in tools_to_run) and state["final_summary"]:
            ...
            try:
                grammar_result = tools["grammar_check"](analysis_state)
        ...
        if (not tools_to_run or "validation_tool" in tools_to_run) and state["final_summary"]:
            ...
            try:
                validation_state = tools["validation_tool"](analysis_state)
        ...
        for tool_name in tools_to_run:
            if tool_name not in ["summarize_interview_history", "grammar_check", "validation_tool"] and tool_name in tools:
                logger.info(f"Running additional tool: {tool_name}")
                try:
                    result = tools[tool_name](analysis_state)
...
def route_after_processing(state: MasterAgentState) -> Literal["run_evaluation_tools", "process_response"]:
    ...
    # If there's an error, log it but continue with the flow
    if state.get("error"):
        logger.warning(f"Error in state, but continuing with the flow: {state.get('error')}")
        # If interview is complete, proceed to evaluation, otherwise continue processing
...
def route_after_evaluation(state: MasterAgentState) -> Literal["generate_final_report"]:
    """Determine the next step after running evaluation tools."""
    # Even if there's an error, we'll proceed to generate_final_report
    # The generate_final_report function will handle the error state
    if state.get("error"):
        logger.warning(f"Error in state, but proceeding to generate_final_report: {state.get('error')}")
    
    return "generate_final_report"


def create_interview_graph(tools=None):
    # Use default tools if none provided
    if tools is None:
        from domains.recruitment.tools import summarize_interview_history, grammar_check, validation_tool
        tools = {
            "summarize_interview_history": summarize_interview_history,
    ...
    # Add nodes with partial application to inject dependencies
    from domains.recruitment.scenario_manager import get_scenario_by_id, select_random_scenario
    from domains.recruitment.conversation import start_interview, process_response as conversation_process_response
    
    # Create node functions with dependencies injected
    async def initialize_with_deps(state):
        # Use function-based implementations directly
        try:
            # Start a new interview with a random scenario
    ...
    async def process_with_deps(state):
        # Use function-based process_response directly
        try:
            if not state.get("candidate_response"):
                logger.warning("No candidate response to process")
                return state
    ...
    async def evaluate_with_deps(state):
        return await run_evaluation_tools(state, tools)
    
    # Add nodes
    graph.add_node("initialize_interview", initialize_with_deps)
    graph.add_node("process_response", process_with_deps)
    graph.add_node("run_evaluation_tools", evaluate_with_deps)
...
# Function to run the interview
async def run_interview(
        candidate_responses: List[str],
        custom_tools=None
) -> Dict[str, Any]:
    # Create the interview graph with custom tools if provided
    interview_graph = create_interview_graph(custom_tools)
    ...
    # Start the interview
    try:
        current_state = await interview_graph.ainvoke(state)
